...
class UniformTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = UniformType::where('company_id' ,$request->get('company_id'))->orderBy('position')->paginate($request->get('paginateCount'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try
        {
            DB::beginTransaction();
            $saved = true;
            $uniformType = UniformType::where('id', $id)->where('company_id' , $request->get('company_id'))->exists();
            if (!$uniformType)
                return $this->errorResponse(trans('messages.not_found'), 404);
            UniformType::where('id', $id)->update([
                'name' => $request->get('name'),
                'code' => $request->get('code'),
                'position' => $request->get('position'),
            ]);
    ...
    {
        return UniformType::where('id', $id)->where('company_id' , $request->get('company_id'))->delete()
            ? $this->successResponse(trans('messages.saved'))
            : $this->errorResponse(trans('messages.not_saved'));
    }
