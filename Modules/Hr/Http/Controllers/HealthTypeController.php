...
use Modules\Hr\Entities\HealthType;
...
class HealthTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = HealthType::where('company_id' , $request->get('company_id'))->paginate($request->get('paginateCount'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try
        {
            DB::beginTransaction();
            $saved = true;
            $healthType = HealthType::where('id', $id)->where('company_id' , $request->get('company_id'))->first(['id']);
            if (!$healthType)
    ...
    {
        return HealthType::where('id', $id)->where('company_id', $request->get('company_id'))->delete()
            ? $this->successResponse(trans('messages.saved'))
            : $this->errorResponse(trans('messages.not_saved'));
    }
