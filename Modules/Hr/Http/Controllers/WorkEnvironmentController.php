...
use Modules\Hr\Entities\WorkEnvironment;
...
class WorkEnvironmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = WorkEnvironment::where('company_id' , $request->get('company_id'))->paginate($request->get('company_id'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try
        {
            DB::beginTransaction();
            $saved = true;
            $workEnvironment = WorkEnvironment::where('id', $id)->where('company_id' , $request->get('company_id'))->first(['id']);
            if (!$workEnvironment)
    ...
    {
        return WorkEnvironment::where('id', $id)->where('company_id', $request->get('company_id'))->delete()
            ? $this->successResponse(trans('messages.saved'))
            : $this->errorResponse(trans('messages.not_saved'));
    }
