...
use Modules\Hr\Entities\RewardType;
...
class RewardTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = RewardType::where('company_id' , $request->company_id)->paginate($request->get('paginateCount'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try
        {
            DB::beginTransaction();
            $saved = true;
            $rewardType = RewardType::where('id', $id)->where('company_id' , $request->get('company_id'))->first(['id']);
            if (!$rewardType)
    ...
    {
        return RewardType::where('id', $id)->where('company_id' , $request->get('company_id'))->delete()
            ? $this->successResponse(trans('messages.saved'))
            : $this->errorResponse(trans('messages.not_saved'));
    }
