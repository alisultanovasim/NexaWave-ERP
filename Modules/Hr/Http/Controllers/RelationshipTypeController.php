...
use Modules\Hr\Entities\RelationshipType;
...
class RelationshipTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = RelationshipType::where('company_id' , $request->company_id)->paginate($request->get('company_id'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try
        {
            DB::beginTransaction();
            $saved = true;
            $relationshipType = RelationshipType::where('id', $id)->first();
            if (!$relationshipType)
    ...
    {
        return RelationshipType::where('id', $id)->delete()
            ? $this->successResponse(trans('messages.saved'))
            : $this->errorResponse(trans('messages.not_saved'));
    }
